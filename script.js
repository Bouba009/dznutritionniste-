// --- 1. FIREBASE CONFIG ---
// استبدل هذا الجزء ببياناتك من Firebase Console
...
let regRole = 'user';
let rateM = null;
let uCache = []; // Cache for admin filtering

// --- 3. TRANSLATIONS (قاموس الترجمة) ---
const tD = {
    ar: {
...
window.onload = async () => {
    ...
    if (uid) {
        try {
            ...
            if (d.exists) { currentUser = { id: d.id, ...d.data() }; initApp(); }
            else { hideLoader(); }
...
function applyLang() {
    ...
    document.querySelectorAll('[data-t]').forEach(e => e.innerText = tD[lang][e.getAttribute('data-t')] || e.innerText);
    document.querySelectorAll('[data-p]').forEach(e => e.placeholder = tD[lang][e.getAttribute('data-p')] || e.placeholder);
}
function t(k) { return tD[lang][k] || k; }
function sAlert(icon, kTitle, kText) {
    let config = { icon: icon, title: t(kTitle), text: t(kText), timer: 3000, showConfirmButton: false };
    if (kTitle === 'reg_ok_pro') { config.iconHtml = '<i class="fas fa-clock fa-3x" style="color:#2196F3"></i>'; config.timer = 5000; }
    Swal.fire(config);
}
function nav(id) {
    ...
    if(id === 'dash-container') document.getElementById('dash-container').classList.remove('hidden');
    toggleSidebar(false);
}
function toggleSidebar(f) {
    const s = document.getElementById('sidebar'), o = document.querySelector('.overlay');
...
function loadCMS() {
    db.collection('settings').doc('config').onSnapshot(s => {
        if (s.exists) {
            const d = s.data();
            if (d.logo) {
                document.getElementById('nav-logo-container').innerHTML = `<img src="${d.logo}">`;
            }
            if (d.bg) {
                document.querySelector('.hero-bg').style.backgroundImage = `url('${d.bg}')`;
            }
        }
    });
}
function trackVisit() {
    ...
    db.collection('stats').doc('visits').set({
        [d]: firebase.firestore.FieldValue.increment(1),
        [d.substring(0, 7)]: firebase.firestore.FieldValue.increment(1)
    }, { merge: true });
}
...
function setRole(r) {
    regRole = r;
    document.getElementById('role-user').className = `role-btn ${r === 'user' ? 'active' : ''}`;
    document.getElementById('role-pro').className = `role-btn ${r === 'pro' ? 'active' : ''}`;
    document.getElementById('pro-fields').classList.toggle('hidden', r !== 'pro');
}
function toggleKids(v) {
    const k = document.getElementById('kids-div');
    if (v === 'Married' || v === 'Divorced') k.classList.remove('hidden'); else k.classList.add('hidden');
}
...
async function doRegister() {
    ...
    if (!n || !e || !p) return Swal.fire('Error', 'Missing Fields', 'error');
    if (p !== p2) return sAlert('error', 'err_title', 'pass_mismatch');

    let d = {
        name: n, email: e, pass: p, role: regRole,
        status: regRole === 'user' ? 'active' : 'pending', joined: Date.now(),
    ...
    if (regRole === 'pro') {
        d.wilaya = document.getElementById('r-wilaya').value;
        d.marital = document.getElementById('r-social').value;
        if (d.marital !== 'Single') d.kids = document.getElementById('r-kids').value;
        d.code = 'T' + Math.floor(10000 + Math.random() * 90000);
        d.lastAccepted = 0; d.points = 0;
        const f = document.getElementById('r-cert').files[0];
        if (f) d.cert = await toBase64(f);
    }

    try {
        await db.collection('users').add(d);
        sAlert(regRole === 'user' ? 'success' : 'info', 'succ_title', regRole === 'user' ? 'reg_ok_user' : 'reg_ok_pro');
        nav('login');
...
function buildLinks() {
    ...
    box.innerHTML += `<div class="menu-item" onclick="navDash()"><i class="fas fa-th-large"></i> ${t('dashboard')}</div>`;
    box.innerHTML += `<div class="menu-item" onclick="openProfile()"><i class="fas fa-user-circle"></i> ${t('profile')}</div>`;
}

function navDash() {
    if(currentUser.role === 'admin') renderAdmin();
    else if(currentUser.role === 'pro') nav('dash-container');
    else nav('dash-container');
}
...
function initUser() {
    ...
    c.innerHTML = `
        <div id="u-slider" style="height:220px;border-radius:15px;overflow:hidden;margin-bottom:20px;box-shadow:var(--shadow)"></div>
        <div class="profile-header">
            <img src="${currentUser.img || 'https://via.placeholder.com/80'}" alt="User">
            <div><h2 style="margin:0">${currentUser.name}</h2><span>${t('welcome')}</span></div>
        </div>
        <h3 class="section-title">${t('available_pros')}</h3>
        <div id="pros-list" class="pro-list" style="margin-bottom:30px"></div>
        <div class="tbl-box"><table><thead><tr><th>${t('pro')}</th><th>${t('status')}</th><th>${t('action')}</th></tr></thead><tbody id="u-hist"></tbody></table></div>
    `;
    ...
    db.collection('users').where('role', '==', 'pro').where('status', '==', 'active').onSnapshot(s => {
        ...
        const now = Date.now();
        // Hide if accepted within 1 hour
        const visible = arr.filter(p => (now - (p.lastAccepted || 0)) > 3600000);
        // Put recently available at bottom
        visible.sort((a, b) => (a.lastAccepted || 0) - (b.lastAccepted || 0));

        visible.forEach(p => {
            const rt = p.rc ? (p.rs / p.rc).toFixed(1) : 0;
            l.innerHTML += `
            <div class="pro-item">
                <img src="${p.img || 'https://via.placeholder.com/80'}">
                <h4>${p.name}</h4>
                <div style="color:gold;cursor:pointer" onclick="viewRev('${p.id}')">⭐ ${Math.round(rt)} (${p.rc || 0})</div>
                <small>${p.wilaya || ''}</small>
                <button class="btn-modern" style="margin-top:10px" onclick="req('${p.id}','${p.name}')">${t('request')}</button>
            </div>`;
        });
    });
    ...
    db.collection('apps').where('uid', '==', currentUser.id).onSnapshot(s => {
        ...
        let hArr = []; s.forEach(d => hArr.push({ id: d.id, ...d.data() }));
        hArr.sort((a, b) => b.date - a.date);
        hArr.forEach(x => {
            let act = `<span>${t('wait')}</span>`;
            if (x.status === 'accepted') {
                act = x.rated ? '✅' : `<button class="btn-modern" style="padding:5px 10px; width:auto" onclick="openRate('${x.pid}','${x.id}')">⭐</button>`;
            } else if (x.status === 'rejected') act = '❌';
            b.innerHTML += `<tr><td>${x.pName}</td><td>${t(x.status)}</td><td>${act}</td></tr>`;
        });
    });
}

async function req(pid, pnm) {
    await db.collection('apps').add({
        uid: currentUser.id, uName: currentUser.name, uAge: currentUser.age, uGen: currentUser.gender,
        pid: pid, pName: pnm, status: 'pending', date: Date.now()
    });
    sAlert('success', 'req_ok_title', 'req_ok_msg');
}
...
function initPro() {
    ...
    const c = document.getElementById('dash-container');
    c.innerHTML = `
        <div id="p-slider" style="height:220px;border-radius:15px;overflow:hidden;margin-bottom:20px;box-shadow:var(--shadow)"></div>
        <div class="profile-header">
            <img src="${currentUser.img || 'https://via.placeholder.com/80'}">
            <div style="flex:1"><h2 style="margin:0">${currentUser.name}</h2><span>Code: ${currentUser.code}</span></div>
            <div class="center-text"><div style="font-size:2rem;font-weight:bold;color:var(--accent)">${currentUser.points || 0}</div><small>${t('points')}</small></div>
        </div>
        <div class="pro-grid">
            <div class="pro-stat pc1"><h3 id="stat-new">0</h3><span>${t('new_req')}</span></div>
            <div class="pro-stat pc2"><h3 id="stat-tot">0</h3><span>${t('total')}</span></div>
            <div class="pro-stat pc3"><h3 id="stat-rej">0</h3><span>${t('rejected')}</span></div>
            <div class="pro-stat pc4"><h3 id="stat-cli">0</h3><span>${t('accepted_clients')}</span></div>
        </div>
        <div class="tbl-box"><table><thead><tr><th>${t('client')}</th><th>${t('age')}</th><th>${t('gender')}</th><th>${t('action')}</th></tr></thead><tbody id="p-req"></tbody></table></div>
        <div class="tbl-box"><table><thead><tr><th>${t('client')}</th><th>${t('phone')}</th><th>${t('email')}</th></tr></thead><tbody id="p-cli"></tbody></table></div>
    `;
    loadBans('p-slider', 'pro'); applyLang();

    db.collection('apps').where('pid', '==', currentUser.id).onSnapshot(s => {
        const r = document.getElementById('p-req'), cl = document.getElementById('p-cli');
        r.innerHTML = ""; cl.innerHTML = "";
        let st = { n: 0, t: 0, r: 0, c: 0 }, cSet = new Set();
        ...
        arr.forEach(x => {
            ...
            if (x.status === 'pending') {
                st.n++;
                r.innerHTML += `<tr><td>${x.uName}</td><td>${x.uAge}</td><td>${t(x.uGen=='Female'?'female':'male')}</td><td><div class="flex-gap"><button class="btn-modern" style="width:auto;padding:5px 10px;background:green" onclick="ans('${x.id}',1)">✔</button> <button class="btn-modern" style="width:auto;padding:5px 10px;background:red" onclick="ans('${x.id}',0)">✖</button></div></td></tr>`;
            } else if (x.status === 'rejected') st.r++;
            else if (x.status === 'accepted') {
                if (!cSet.has(x.uid)) {
                    cSet.add(x.uid);
                    db.collection('users').doc(x.uid).get().then(u => {
                        const ud = u.data();
                        cl.innerHTML += `<tr><td>${ud.name}</td><td>${ud.phone}</td><td>${ud.email}</td></tr>`;
                    });
                }
            }
        });
        document.getElementById('stat-new').innerText = st.n;
        document.getElementById('stat-tot').innerText = st.t;
        document.getElementById('stat-rej').innerText = st.r;
        document.getElementById('stat-cli').innerText = cSet.size;
    });
}

async function ans(aid, ok) {
    await db.collection('apps').doc(aid).update({ status: ok ? 'accepted' : 'rejected' });
    if (ok) await db.collection('users').doc(currentUser.id).update({ lastAccepted: Date.now() });
}
...
function renderAdmin() {
    ...
    c.innerHTML = ""; c.appendChild(tpl);
    applyLang();

    // Stats Listeners
    db.collection('users').onSnapshot(s => {
        document.getElementById('adm-u').innerText = s.size;
        const pending = s.docs.filter(d => d.data().status === 'pending').length;
        document.getElementById('adm-w').innerText = pending;
        document.getElementById('adm-p').innerText = s.docs.filter(d => d.data().role === 'pro').length;

        const tb = document.querySelector('#tbl-users tbody'); tb.innerHTML = "";
        const tbA = document.querySelector('#tbl-apps tbody'); tbA.innerHTML = "";

        s.forEach(doc => {
            const u = doc.data();
            // User Table
            if (u.role !== 'admin') {
                tb.innerHTML += `<tr><td>${u.name}</td><td>${u.role}</td><td><button class="btn-del" onclick="delUser('${doc.id}')">Delete</button></td></tr>`;
            }
            // Approvals Table
            if (u.status === 'pending') {
                const certBtn = u.cert ? `<button class="btn-modern" style="padding:5px 10px; width:auto; font-size:0.8rem" onclick="viewCert('${u.cert}')">View</button>` : '-';
                tbA.innerHTML += `<tr><td>${u.name}</td><td>${certBtn}</td><td><div class="flex-gap"><button class="btn-modern" style="width:auto; padding:5px; background:green" onclick="approve('${doc.id}',1)">✔</button><button class="btn-modern" style="width:auto; padding:5px; background:red" onclick="approve('${doc.id}',0)">✖</button></div></td></tr>`;
            }
        });
    });

    db.collection('reports').onSnapshot(s => {
        document.getElementById('adm-r').innerText = s.size;
        const tb = document.querySelector('#tbl-reps tbody'); tb.innerHTML = "";
        s.forEach(d => {
            const r = d.data();
            tb.innerHTML += `<tr><td>${r.name}</td><td>${r.email}</td><td>${r.desc}</td><td><button class="btn-del" onclick="delRep('${d.id}')">Del</button></td></tr>`;
        });
    });

    db.collection('apps').where('status', '==', 'accepted').onSnapshot(s => {
        document.getElementById('adm-s').innerText = s.size;
        const clients = new Set(s.docs.map(d => d.data().uid)).size;
        document.getElementById('adm-c').innerText = clients;
        const tb = document.querySelector('#tbl-sess tbody'); tb.innerHTML = "";
        s.forEach(d => {
            const x = d.data();
            tb.innerHTML += `<tr><td>${x.uName}</td><td>${x.pName}</td><td>${new Date(x.date).toLocaleDateString()}</td></tr>`;
        });
    });

    // Visits
    db.collection('stats').doc('visits').onSnapshot(d => {
        if (d.exists) {
            const dt = d.data();
            const now = new Date().toISOString().split('T')[0];
            document.getElementById('adm-vd').innerText = dt[now] || 0;
            document.getElementById('adm-vm').innerText = dt[now.substring(0, 7)] || 0;
        }
    });

    // Banners
    db.collection('banners').onSnapshot(s => {
        const bl = document.getElementById('ban-list'); bl.innerHTML = "";
        s.forEach(d => {
            bl.innerHTML += `<div style="display:flex; justify-content:space-between; align-items:center; padding:5px; border-bottom:1px solid #eee"><img src="${d.data().img}" width="50" height="30" style="object-fit:cover"><button class="btn-del" onclick="delBan('${d.id}')">Del</button></div>`;
        });
    });
}

function showAdm(id) {
    document.querySelectorAll('[id^="adm-view-"]').forEach(e => e.classList.add('hidden'));
    document.getElementById('adm-view-' + id).classList.remove('hidden');
}
...
function viewCert(c) { const w = window.open(); w.document.write(`<img src="${c}" style="max-width:100%">`); }
...
// --- UTILS ---
function checkNotifs() { const r = currentUser.role == 'admin' ? 'admin' : currentUser.role; db.collection('notifs').where('target', 'in', ['all', r]).onSnapshot(s => { let c = 0; s.forEach(d => { if (!localStorage.getItem('rn_' + d.id)) c++ }); const b = document.getElementById('notif-count'); b.innerText = c; b.style.display = c > 0 ? 'block' : 'none'; }); }
function showNotifs() { const r = currentUser.role == 'admin' ? 'admin' : currentUser.role; db.collection('notifs').where('target', 'in', ['all', r]).get().then(s => { let h = ""; s.forEach(d => { localStorage.setItem('rn_' + d.id, '1'); h += `<div style="padding:10px;border-bottom:1px solid #eee">${lang == 'ar' ? d.data().ar : d.data().fr}</div>` }); document.getElementById('inf-t').innerText = t('report'); document.getElementById('inf-c').innerHTML = h || "Empty"; document.getElementById('mod-info').style.display = 'flex'; document.getElementById('notif-count').style.display = 'none'; }); }
